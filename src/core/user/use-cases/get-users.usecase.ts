...
const Query = z.object({
  ...
  page: z.string().optional().transform((val) => val ? parseInt(val) : 1).refine((val) => val !== 0, {message: 'page not is zero'}),
  limit: z.string().optional().transform((val) => val ? parseInt(val) : 10).refine((val) => val !== 0, {message: 'limit not is zero'})
})
const execute =
...
export const GetUsersUseCase: UseCase<Dependencies, {		
  orderBy?: string
  page?: string
  limit?: string
  search?: string
